...
public class CartDB {
	...
	public void addToCart(int userid, int productid, int quantity) {
		try (Connection conn = DBConnection.getCon()) {
			PreparedStatement statement = conn.prepareStatement("insert into cart values(?,?,?);");
	...
	public ArrayList<Products> findcartByuserId(int id) {
		...
		try (Connection conn = DBConnection.getCon()) {
			...
			ResultSet rs = stmt.executeQuery(
					"select * from products where productId in(select productID from cart Where userID =" + id + ");");
